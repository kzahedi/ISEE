...
public class EvolutionSaxHandler extends DefaultHandler
{
  ...
  public EvolutionSaxHandler()
  {
    super();
    progressMonitor = new ProgressMonitor(
        null, "Loading Net", "Generation...", 0, 100);
  ...
  public Vector readGeneration(String filename,
      int generationIndex, 
      int populationIndex)
  {
    synapseList = new Vector();
    netVector = new Vector();
    _wanted[INDIVIDUAL_INDEX] = -1;
    _wanted[GENERATION_INDEX] = generationIndex;
    _wanted[POPULATION_INDEX] = populationIndex;
    progressMonitor.setMaximum(generationIndex);
    try
  ...
  {
    _neuronIndex++;
    double bias = Double.parseDouble(attributes.getValue("Bias").trim());
    ...
    {
      transmitter =
        Double.parseDouble(attributes.getValue("TransmitterLevel").trim());
    }
    double receptors = 0;
    if(attributes.getValue("ReceptorLevel") != null)
    {
      receptors = Double.parseDouble(attributes.getValue("ReceptorLevel").trim());
    }
    if(attributes.getValue("TransmitterLevel") != null)
    {
      transmitter =
  ...
  public void addSynapse(Attributes attributes)
  {
    ...
    int sourceIndex = Integer.parseInt(attributes.getValue("Source").trim());
    double strength = Double.parseDouble(attributes.getValue("Strength").trim());
    String processModeString = attributes.getValue("Process");
  ...
  {
    EvolutionSaxHandler evolutionSaxHandler = new EvolutionSaxHandler();
    ...
    {
      Vector nets = evolutionSaxHandler.readGeneration(filename, generationIndex,
          populationIndex);
    ...
    {
      Net net = evolutionSaxHandler.readNet(filename, generationIndex, populationIndex,
          individualIndex);
      if(net == null)
      {
