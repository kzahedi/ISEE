...
public class NetTreeView extends JFrame implements ActionListener
{
  ...
  private int viewMode = MODE_VIEW_NET;


  private Net net = null;
  
  public NetTreeView(int mode)
  {
    // **************************************************************************
    // general stuff
    // **************************************************************************
    ...
    switch(mode)
    {
      case MODE_EDIT_NET:
        ...
        tabPane.addTab("Add Neuron",null,neuronAddPanel,"Add a neuron to the net");
        tabPane.addTab("Del Neuron",null,neuronDelPanel,"Delete a neuron from the"
            + " net");
        tabPane.addTab("Change Neuron",null,neuronChangePanel,"Change an existing"
            + " neuron");
        tabPane.addTab("Add/Change Synaspe",null,synapseAddPanel,"Add a synapse to"
            + " the net or change the value of an existing synapse");
        tabPane.addTab("Del Synapse",null,synapseDelPanel,"Delete a synapse from"
            + " the net");
        tabPane.addTab("General",null,globalPanel,"New Net/Save Net/Close"
            + " Application");
        setupNeuronPanel();
        setupSynapsePanel();
  ...
  public void actionPerformed(ActionEvent e)
  {
    ...
    if(e.getSource() == saveButton)
    {
      ...
      if(returnVal == JFileChooser.APPROVE_OPTION) {
        ...
        eh.writeNetToFile(net, outputfilename, "Created with NetTreeView.");
      }
    }
    ...
    if(e.getSource() == addSynapseButton)
    {
      ...
      {
        System.out.println("NetTreeView:: calling setStrength");
  ...
  public void updateView(Net net)
  {
    ...
    this.net = net;
    updateNeuronSelectionComboBoxes();
    DefaultMutableTreeNode dummy = net.toJTreeNode();
    if(dummy != null)
    {
  ...
  public static void main(String argv[])
  {
    ...
    NetTreeView netTreeView = new NetTreeView();
    netTreeView.setVisible(true);

    NetTreeView netTreeView2 = new NetTreeView(NetTreeView.MODE_VIEW_NET);
